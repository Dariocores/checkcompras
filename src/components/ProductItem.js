...
import { format, CATEGORIES } from "@/utils/constants";
...
const s = {
  ...
  editGroup: { display: "flex", flexDirection: "column", gap: 4 },
  editRow: { display: "flex", gap: 4 },
  ...
  editSelect: { background: "var(--surface2)", color: "var(--text)", padding: "6px 8px", borderRadius: "var(--radius-sm)", fontSize: 14, border: "none", outline: "none" },
        ...
        {isEditing ? (
          <div style={s.editGroup}>
            <div style={s.editRow}>
              ...
              <input type="number" defaultValue={p.price} step="0.01" min="0.01" onBlur={(e) => onFinishEdit(p.id, "price", e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") onFinishEdit(p.id, "price", e.target.value); if (e.key === "Escape") onCancelEdit(); }} style={{ ...s.editInput, flex: "none", width: 80 }} />
            </div>
            <select defaultValue={p.category} onBlur={(e) => onFinishEdit(p.id, "category", e.target.value)} onChange={(e) => onFinishEdit(p.id, "category", e.target.value)} style={s.editSelect}>
